...
unsigned short **my_str_to_word_array(const char *str, const char *separator)
{
    int j = 0;
    int i = 0;
    unsigned short **final_array = malloc((sizeof(unsigned short *)) \
    * get_nb_words(str, separator) + 1);
    ...
    while (*str != '\0') {
        ...
        && is_separator(*str, separator) == 0)) {
            final_array[j] = malloc((sizeof(unsigned short) \
